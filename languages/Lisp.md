...
(incf x 1) = (setf x (1+ x)) = (setf x (+ 1 x))
(defc x 1) ...
```
...
;; Optional arguments
(defun fn (a b &optional (c 10) &rest d &key e (f 30 f-supplied-p))   ;; f-supplied-p is T
     ...)                                                                                                                 ;; if f was given

(fn 1 2)                  a = 1, b = 2, c = 10, e = nil, f = 30
(fn 1 2 3)               c = 3, ...
(fn 1 2 3 4 5)        ;; ERROR: &key restricts what can be passed to &rest
                              ;;                 unless you add &allow-other-keys
(fn 1 2 :e 3)           ;; ERROR: LISP tries to match c with :e!!! /!\

;; Local functions (let for functions)
(flet ((<fun1> (<args>)                                    ; flet* -> labels (!)
          ...)
          (<fun2> (<args2>)
          ...)
         )
...
(format t "Hello ~A" x)           Like printf. ~A is anything, ~% is newline, 
                                                     ~& is newline if not blank line already, ~~ is tilde
(format nil ...)                            Return string (like sprintf)
(format <stream> ...)               Print to stream (ex: file)
(read [<stream> <eof-error-p> <eof-value>])        Read from stream, optionally error on
                                                                                       EOF or instead return a specific value.
(read)                                                 Read from stdin
(read-line ...)                                    Read whole line
(read-from-string <string> ...)     Read from string
(with-open-file (file "filename.txt" :direction (:input|:output)                   File I/O
...
## Macros
```lisp
(defmacro <name> (arg1 arg2)               ` Forbids evaluation
     `(dosomething-with ,arg1 ,arg2))        except of expressions with , before

(defmacro avg (&rest args)                      @args to pull them out of list
    `(/ (+ ,@args) ,(length args)))

(defmacro for (var start stop &body bd)           When aux vars are used, gensym
    (let ((gstop (gensym)))                                       is required to avoid overwriting
        `(do ((,var ,start (1+ ,var))                                possible vars with same name
                 (,gstop ,stop))
               ((>= ,var ,gstop))
               ,@ bd)))
```
...
(defun |$-reader| (stream char)                       |$-reader| recursively reads stream and
    (list 'dollars (read stream T nil T)))            applies the dollars function to arguments

(set-dispatch-macro-character ...)               Creates family of macro characters with
                                                                               optional arguments: #1.1$20
```                                                                               
...
(defmethod <name> ((a1 <class>) a2)          Specializes a method to a class
    ...

(defmethod <name> :before ((a1 <class>) a2)      Method to be called before the other
    ...                                                                                 specializations

(defmethod <name> :after ((a1 <class>) a2)         Method to be called before the other
    ...                                                                                 specializations

(defmethod <name> :around ((a1 <class>) a2)    Method to be called around the other
    ... (let ((res (call-next-method)))                         specializations. May return another 
            ...                                                                         value
            res))
